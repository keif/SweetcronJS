	...
	query.exec(function (err, vote) {
		...
		if (vote.length) {
			...
			Vote.update(voteObj, {
				...
				// default
				downvote = (num === -1);
				hasUpvote = (originalVote === 1);
				hasDownvote = (originalVote === -1);
				upvote = (num === 1);
				...
				req.post.upvote(newUpvote, function (err) {
					if (err) {
						return next(err);
					}
...
router.post('/posts/:post/comments', auth, function (req, res, next) {
	...
	comment.save(function (err, comment) {
		...
		// req.post.save(function (err, comment, post) {
